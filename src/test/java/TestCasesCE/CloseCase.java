...
public class CloseCase extends ForcedAbatement {
	...
	public static void CloseCase_PreRequisite() throws InterruptedException {
		...
		extentTest.setDescription(
				" Verify that 'Close Case' option appears, when user clicks on 'More' button, on CDP. ");
...
//		    WebDriverWaits.ClickByJsExecuter(StatusColumnSort);
//			Thread.sleep(8000);
//			WebDriverWaits.ClickByJsExecuter(ClosedCase);
		}
		else {
	...
	public static void CloseCase_OpenCloseCasePopup() throws InterruptedException {
		...
		extentTest.setDescription(
				" Verify that 'Close Case' popup opens up, when user selects 'Close Case' option, after clicking on 'More' button, on CDP. ");
		...
		s135.assertAll();
		// Assert.assertEquals(CCPopupText, "Close Case");
	...
	public static void CloseCase_CloseCloseCasePopup() throws InterruptedException {
		...
		extentTest.setDescription(
				" Verify that 'Close Case' popup gets closed, when user clicks on 'Cross' icon or 'Cancel' button, on 'Verify Violations' section. ");
	...
	public static void CloseCase_VerifyClosedStatus() {
		...
		extentTest.setDescription(
				" Verify that 'Closed' status appears after Case gets closed, when user clicks on 'Close Case' button, on 'Close Case' popup. ");
		try {
...
//			WebDriverWaits.SendKeys(NameFieldPIP, ContactName);
	...
	public static void CloseCase_CloseConfirmationPopupForCloseCase() {
		extentTest = extent.startTest(" CloseCase_ConfirmationPopupForCloseCaseInvalidViolations ");
		extentTest.setDescription(
				" Verify that confirmation popup gets closed, when user clicks on 'Cancel' button, under 'Verify Violations' section. ");
		try {
			Thread.sleep(4000);
	...
	public static void CloseCase_VerifyClosedStatusViaPerformInspection() {
		...
		extentTest.setDescription(
				" Verify that 'Closed' status appears after Case gets closed, when user clicks on 'Yes' button, on 'Close Case' popup. ");
		try {
			Thread.sleep(3000);
			WebDriverWaits.ClickOn(FooterButtons);
			Thread.sleep(4000);
			...
			s143.assertAll();
			// Assert.assertEquals(ClosedStatus2, "Closed");
		} catch (InterruptedException e) {
			e.printStackTrace();
...
//		WebDriverWaits.ClickOn(CheckboxSchIns);
	...
	public static void CloseCase_CloseCasewithValidViolations() {
		...
		extentTest.setDescription(
				" Verify that user is able to close the case for valid violations, after clicking on 'Complete Inspection & Close Case' button > 'Close Case' button, on 'Close Case' popup. ");
		try {
